...
void ESP8266SoftwareSerial::setup(InternalGPIOPin *tx_pin, InternalGPIOPin *rx_pin, uint32_t baud_rate,
  ...
  this->bit_time_ = arch_get_cpu_freq_hz() / baud_rate;
  ...
  //gpio_test_pin_ = new esp8266::ESP8266GPIOPin();
  //gpio_test_pin_->set_pin(14);
  //gpio_test_pin_->set_inverted(false);
  //gpio_test_pin_->set_flags(gpio::Flags::FLAG_OUTPUT);
  //gpio_test_pin_->setup();
}
void IRAM_ATTR ESP8266SoftwareSerial::gpio_intr(ESP8266SoftwareSerial *arg) {
  unsigned long deadline = arch_get_cpu_cycle_count() + arg->bit_time_ - 100;
  uint8_t rec = 0;
  // Manually unroll the loop
  for (int i = 0; i < arg->data_bits_; i++)
    rec |= arg->read_bit_(&deadline) << i;
...
void IRAM_ATTR HOT ESP8266SoftwareSerial::write_byte(uint8_t data) {
  ...
  {
    InterruptLock lock;
    unsigned long deadline = arch_get_cpu_cycle_count() + this->bit_time_;

    // Start bit
    this->write_bit_(false, &deadline);
    for (int i = 0; i < this->data_bits_; i++) {
      bool bit = data & (1 << i);
      this->write_bit_(bit, &deadline);
      if (need_parity_bit)
        parity_bit ^= bit;
    }
    if (need_parity_bit)
      this->write_bit_(parity_bit, &deadline);
    // Stop bit
    this->write_bit_(true, &deadline);
    if (this->stop_bits_ == 2)
      this->wait_(&deadline);
  }
}
void IRAM_ATTR ESP8266SoftwareSerial::wait_(unsigned long *deadline) {
  while (arch_get_cpu_cycle_count() < *deadline)
    ;
  *deadline += this->bit_time_;
}
bool IRAM_ATTR ESP8266SoftwareSerial::read_bit_(unsigned long *deadline) {
  this->wait_(deadline);
  return this->rx_pin_.digital_read();
  //this->gpio_test_pin_->digital_write(1);
  //bool value = this->rx_pin_.digital_read();
  //this->gpio_test_pin_->digital_write(0);
  //return value;
}
void IRAM_ATTR ESP8266SoftwareSerial::write_bit_(bool bit, unsigned long *deadline) {
