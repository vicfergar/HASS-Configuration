...
bool TuyaLight::shouldIgnoreDimmerCommand()
{
  ...
  {
      ESP_LOGD(TAG, "dimmer_cmd: ignored");
      return true;
  }
...
void TuyaLight::write_state(light::LightState *state) {
  if (this->ignore_write_state)
  {
      ...
      ESP_LOGD(TAG, "write_state: ignored");
      return;
  }
  ...
  this->ignore_dimmer_cmd_timeout = millis() + 500; // Ignore serial received dim commands for the next 500ms
